...
const ourTeams = [
  ...
  },
  {
    ...
    teamProfileSnap: "/about-us/prof-dari-kumar.jpg",
            ...
            ourTeams.map((item, index)=> {
              return(
                <li key={index} className='w-full sm:w-1/2 lg:w-1/4 lg:last:mx-[30%] p-4'>
                  <Link href={item.link} className='group block relative w-full h-80 md:h-64 lg:h-92 xl:h-[20rem] 2xl:h-[25rem]'>
                    <Image 
                      src={item.teamProfileSnap}
                      alt={item.name}
                      fill
                      className='object-center object-cover rounded-tr-[5rem] grayscale hover:grayscale-0 duration-300'
                    />
                    <div className='absolute bottom-0 left-0 px-5 py-2 bg-[rgba(0,0,0,0.6)] m-2 flex flex-col items-start justify-start opacity-0 duration-300 group-hover:opacity-100'>
                      <p className='text-white text-justify text-sm line-clamp-3'>{item.about}</p>
                      <span className=' bg-white font-bold px-3 py-1 mt-2'>
                        Know More 
